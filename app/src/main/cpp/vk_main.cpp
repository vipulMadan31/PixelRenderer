...
// Global pointer so JNI can access the Vulkan backend for metadata queries
static vkt::HelloVK *g_vulkanBackend = nullptr;
...
/*
 * JNI function to retrieve metadata JSON from the Vulkan backend.
 * Called from Kotlin to get shader sizes, GPU info, etc.
 */
extern "C" JNIEXPORT jstring JNICALL
Java_com_android_hellovk_VulkanActivity_nativeGetMetadataJson(
    JNIEnv *env, jobject /* this */) {
  if (g_vulkanBackend != nullptr && g_vulkanBackend->initialized) {
    std::string json = g_vulkanBackend->getMetadataJson();
...
void android_main(struct android_app *state) {
  ...
  // Set global pointer for JNI access
  g_vulkanBackend = &vulkanBackend;
