...
const int RENDER_PASSES_PER_FRAME = 80;
...
class HelloVK {
 ...
 private:
  ...
  VkRenderPass renderPassLoad;  // render pass that loads (doesn't clear) for multi-pass
...
void HelloVK::createDescriptorSetLayout() {
  // No descriptors needed — shaders use hardcoded values
}
...
void HelloVK::recordCommandBuffer(VkCommandBuffer commandBuffer,
  ...
  // Render into the full swapchain extent (full screen)
  const uint32_t RENDER_WIDTH  = swapChainExtent.width;
  const uint32_t RENDER_HEIGHT = swapChainExtent.height;
  ...
  // Multi-pass rendering: run RENDER_PASSES_PER_FRAME iterations
  // to keep the GPU busy with full-screen fragment shader work.
  for (int pass = 0; pass < RENDER_PASSES_PER_FRAME; pass++) {
    VkRenderPassBeginInfo renderPassInfo{};
    renderPassInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
    // First pass clears the framebuffer; subsequent passes load it
    renderPassInfo.renderPass = (pass == 0) ? renderPass : renderPassLoad;
...
void HelloVK::pickPhysicalDevice() {
  ...
  // Cache device properties for metadata export
...
void HelloVK::createRenderPass() {
  // --- Render pass 1: CLEAR (used for the first pass each frame) ---
  VkAttachmentDescription colorAttachmentClear{};
  colorAttachmentClear.format = swapChainImageFormat;
  colorAttachmentClear.samples = VK_SAMPLE_COUNT_1_BIT;
  colorAttachmentClear.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
  colorAttachmentClear.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
  colorAttachmentClear.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
  colorAttachmentClear.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
  colorAttachmentClear.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
  colorAttachmentClear.finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
  ...
  VkRenderPassCreateInfo renderPassInfoClear{};
  renderPassInfoClear.sType = VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO;
  renderPassInfoClear.attachmentCount = 1;
  renderPassInfoClear.pAttachments = &colorAttachmentClear;
  renderPassInfoClear.subpassCount = 1;
  renderPassInfoClear.pSubpasses = &subpass;
  renderPassInfoClear.dependencyCount = 1;
  renderPassInfoClear.pDependencies = &dependency;

  VK_CHECK(vkCreateRenderPass(device, &renderPassInfoClear, nullptr, &renderPass));

  // --- Render pass 2: LOAD (used for passes 1..N-1 that overdraw) ---
  VkAttachmentDescription colorAttachmentLoad{};
  colorAttachmentLoad.format = swapChainImageFormat;
  colorAttachmentLoad.samples = VK_SAMPLE_COUNT_1_BIT;
  colorAttachmentLoad.loadOp = VK_ATTACHMENT_LOAD_OP_LOAD;
  ...
  VkRenderPassCreateInfo renderPassInfoLoad{};
  renderPassInfoLoad.sType = VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO;
  renderPassInfoLoad.attachmentCount = 1;
  renderPassInfoLoad.pAttachments = &colorAttachmentLoad;
  renderPassInfoLoad.subpassCount = 1;
  renderPassInfoLoad.pSubpasses = &subpass;
  renderPassInfoLoad.dependencyCount = 1;
  renderPassInfoLoad.pDependencies = &dependency;

  VK_CHECK(vkCreateRenderPass(device, &renderPassInfoLoad, nullptr, &renderPassLoad));

  LOGI("Created dual render passes: clear + load (multi-pass count = %d)", RENDER_PASSES_PER_FRAME);
}
...
void HelloVK::createGraphicsPipeline() {
  ...
  // Store shader sizes for metadata export
  vertShaderSize = vertShaderCode.size();
  fragShaderSize = fragShaderCode.size();
  LOGI("Shader SPIR-V sizes: vert=%zu bytes, frag=%zu bytes", vertShaderSize, fragShaderSize);
...
std::string HelloVK::getMetadataJson() {
  ...
  json << "{\n";
  json << "  \"gpu_device_name\": \"" << deviceProperties.deviceName << "\",\n";
  json << "  \"vulkan_api_version\": \"" << apiMajor << "." << apiMinor << "." << apiPatch << "\",\n";
  json << "  \"gpu_vendor_id\": " << deviceProperties.vendorID << ",\n";
  json << "  \"gpu_device_id\": " << deviceProperties.deviceID << ",\n";
  json << "  \"gpu_driver_version\": " << deviceProperties.driverVersion << ",\n";
  json << "  \"vertex_shader_spirv_size_bytes\": " << vertShaderSize << ",\n";
  json << "  \"fragment_shader_spirv_size_bytes\": " << fragShaderSize << ",\n";
  json << "  \"render_passes_per_frame\": " << RENDER_PASSES_PER_FRAME << ",\n";
  json << "  \"swapchain_image_count\": " << swapChainImages.size() << ",\n";
  json << "  \"swapchain_image_format\": " << swapChainImageFormat << ",\n";
  json << "  \"swapchain_extent_width\": " << swapChainExtent.width << ",\n";
  json << "  \"swapchain_extent_height\": " << swapChainExtent.height << ",\n";
  json << "  \"max_frames_in_flight\": " << MAX_FRAMES_IN_FLIGHT << ",\n";
  json << "  \"vertices_per_draw\": 6,\n";
  json << "  \"total_draw_calls_per_frame\": " << RENDER_PASSES_PER_FRAME << "\n";
  json << "}";
